const PLAYER = {
  ...
  // Add resize handler to prevent refresh
  window.addEventListener('resize', function(e) {
      e.preventDefault();
      if (document.getElementById('game-board').classList.contains('hidden')) {
          return; // Don't redraw if game hasn't started
      }
      ...
      for (let i = 0; i < 7; i++) {
          for (let j = 0; j < 7; j++) {
              if (positionMatrix[i][j] !== 0 && positionMatrix[i][j] !== -1) {
                  let [xCenter, yCenter] = getCoordinates(i, j);
                  if (xCenter && yCenter) {
                      context.beginPath();
                      context.arc(xCenter, yCenter, blockWidth, 0, 2 * Math.PI, false);
      ...
      // Redraw selected piece if exists
      if (selectedPiece) {
          let [xCenter, yCenter] = getCoordinates(selectedPiece.x, selectedPiece.y);
          if (xCenter && yCenter) {
              context.beginPath();
              context.arc(xCenter, yCenter, blockWidth, 0, 2 * Math.PI, false);
              context.fillStyle = currentPlayer === PLAYER.HUMAN ? '#AED581' : '#FFCDD2';
